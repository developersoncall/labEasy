...
import { FaPlus, FaTrash, FaMagic, FaGripVertical } from 'react-icons/fa';
...
const emptyParam = { name: '', unit: '', refRange: '', refLow: '', refHigh: '', groupLabel: '' };
  ...
  useEffect(() => {
    ...
    labTestService
      ...
      .then(({ parameters, schemaMissing: missing }) => {
        if (!alive) return;
        setParams(parameters.map((p) => ({ ...p, refLow: p.refLow ?? '', refHigh: p.refHigh ?? '' })));
